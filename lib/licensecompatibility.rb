...
module Licenserec
  class CompatibilityFilter
    def initialize(repo_path,ninka_path,recommand_scale="popular")
      puts "兼容许可证筛选"
      licenseA_set = CompatibilityFilter.license_detection(repo_path,ninka_path)
      licenseA_set.each do |ii|
        puts ii
      end
      rec1,rec2,rec12 = CompatibilityFilter.compatibility_filter(repo_path,ninka_path,recommand_scale)
      puts "rec1 ----"
      puts rec1
      puts "rec2 ---"
      puts rec2
    ...
    def self.compatibility_filter(repo_path,ninka_path,recommand_scale="popular")
      ...
      if recommand_scale == "popular"
        lisenseB_list = %w[MIT Apache-2.0 GPL-3.0 BSD-3-Clause GPL-2.0 AGPL-3.0 MPL-2.0 LGPL-3.0 BSD-2-Clause Unlicense ISC EPL-1.0 CC0-1.0 LGPL-2.1 WTFPL Zlib EPL-2.0 Artistic-2.0 MulanPSL-2.0 MulanPubL-2.0]
      elsif recommand_scale == "all"
        c_table = CSV.read("lib\\compatibility_63.csv",headers:true)
        lisenseB_list = c_table['license']
      else
  ...
  class CompatibilityCheck
    def initialize(repo_path,ninka_path)
      puts "兼容性冲突检查"
      s1,s2 = CompatibilityCheck.compatibilitycheck(repo_path,ninka_path)
      s1.each do |ii|
        puts ii
      end
      s2.each do |ii|
